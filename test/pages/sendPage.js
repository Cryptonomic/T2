...
class SendPage extends BasePage {
    constructor(app) {
        ...
        this.changeFeeLevel = async (feeLevel) => {
            ...
            switch (feeLevel) {
                case "Low":
                    await this.app.client.click(this.sendLowFee); // take lowest fee
                    const selectedFee = await this.app.client.getText(this.selectedFeeValue)
                    assert.equal(selectedFee.includes("Low Fee"), true)
                    break;
                case "Medium":
                    await this.app.client.click(this.sendMediumFee); // take lowest fee
                    break;
                case "High":
                    await this.app.client.click(this.sendHighFee); // take lowest fee
                    break;
                default:
        ...
        this.fillSendForm = async ({
            ...
            if (amount) {
                if (amount = "Max") { await app.client.click(this.sendUseMaxButton) }
                else {
