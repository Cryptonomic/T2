...
export async function getKumulusCollection(tokenMapId: number, managerAddress: string, node: Node, skipDetails: boolean = false): Promise<GetNFTCollection> {
    ...
    const collection = await Promise.all(
        collectionResult.map(async (row) => {
            ...
            if (!skipDetails) {
                try {
                    objectDetails = await getKumulusNFTObjectDetails(node.tezosUrl, objectId);
...
export async function getKumulusNFTObjectDetails(tezosUrl: string, objectId: number) {
    const packedNftId = TezosMessageUtils.encodeBigMapKey(Buffer.from(TezosMessageUtils.writePackedData(objectId, 'int'), 'hex'));
    const nftInfo = await TezosNodeReader.getValueForBigMapKey(tezosUrl, 19922, packedNftId); // TODO: store in token definition
    const ipfsUrlBytes = JSONPath({ path: '$.args[1][0].args[1].bytes', json: nftInfo })[0];
    const ipfsHash = Buffer.from(ipfsUrlBytes, 'hex').toString().slice(7);
...
export async function getGogoCollection(tokenMapId: number, managerAddress: string, node: Node, skipDetails: boolean = false): Promise<GetNFTCollection> {
    ...
    const collection = await Promise.all(
        collectionResult.map(async (row) => {
            ...
            if (!skipDetails) {
                try {
                    objectDetails = await getGogoNFTObjectDetails(node.tezosUrl, objectId);
...
export async function getGogoNFTObjectDetails(tezosUrl: string, objectId: number) {
    const packedNftId = TezosMessageUtils.encodeBigMapKey(Buffer.from(TezosMessageUtils.writePackedData(objectId, 'int'), 'hex'));
    const nftInfo = await TezosNodeReader.getValueForBigMapKey(tezosUrl, 20611, packedNftId); // TODO: store in token definition
    const ipfsUrlBytes = JSONPath({ path: '$.args[1][0].args[1].bytes', json: nftInfo })[0];
    const ipfsHash = Buffer.from(ipfsUrlBytes, 'hex').toString().slice(7);
...
export async function getNeonzCollection(tokenMapId: number, managerAddress: string, node: Node, skipDetails: boolean = false): Promise<GetNFTCollection> {
    ...
    const collection = await Promise.all(
        collectionResult.map(async (row) => {
            ...
            if (!skipDetails) {
                try {
                    objectDetails = await getNeonzNFTObjectDetails(node.tezosUrl, objectId);
...
export async function getNeonzNFTObjectDetails(tezosUrl: string, objectId: number) {
    const packedNftId = TezosMessageUtils.encodeBigMapKey(Buffer.from(TezosMessageUtils.writePackedData(objectId, 'int'), 'hex'));
    const nftInfo = await TezosNodeReader.getValueForBigMapKey(tezosUrl, 21220, packedNftId); // TODO: store in token definition
    const ipfsUrlBytes = JSONPath({ path: '$.args[1][0].args[1].bytes', json: nftInfo })[0];
    const ipfsHash = Buffer.from(ipfsUrlBytes, 'hex').toString().slice(7);
