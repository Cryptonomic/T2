                    ...
                    <TosContainer>
                        ...
                        <FormGroupWrapper>
                            <FormControlLabelWrapper
                                ...
                                label={t('components.landingCarousel.agree_terms') as string}
                            />
                            <FormControlLabelWrapper
                                control={<CheckBoxWrapper checked={isPP} onChange={(e) => setIsPP(e.target.checked)} name="checkedB" color="primary" />}
                                label={t('components.landingCarousel.agree_policy') as string}
                            />
                        </FormGroupWrapper>
                    </TosContainer>
                );
            case 2:
                return <ThirdTxt>{t('components.landingCarousel.galleon_access_des') as string}</ThirdTxt>;
            default:
                return <ThirdTxt>{t('components.landingCarousel.guard_password_des') as string}</ThirdTxt>;
        }
    }
    ...
    function getSlides() {
        return SLIDESLIST.map((index) => {
            return (
                <Container key={index}>
                    <Container1>
                        <WelcomeTxt>{t('components.termsModal.welcome_to', { name: config.name }) as string}</WelcomeTxt>
                        {index === 0 && <LangTitle>{t('components.languageSelectModal.select_language') as string}</LangTitle>}
