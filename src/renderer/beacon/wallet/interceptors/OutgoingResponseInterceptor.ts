...
import { AppMetadataManager, getAccountIdentifier, Logger, PermissionManager } from '../../core';
...
export class OutgoingResponseInterceptor {
    ...
    private static async handleV3Message(config: OutgoingResponseInterceptorOptions) {
        ...
        const wrappedMessage: BeaconMessageWrapper<PermissionResponseV3<string>> | BeaconMessageWrapper<BlockchainResponseV3<string>> = msg as any;

        const v3Message: PermissionResponseV3<string> | BlockchainResponseV3<string> = wrappedMessage.message;
    ...
    private static async handleV2Message(config: OutgoingResponseInterceptorOptions) {
        const { senderId, request, message, ownAppMetadata, permissionManager, appMetadataManager, interceptorCallback }: OutgoingResponseInterceptorOptions =
            config;
        ...
        switch (message.type) {
            ...
            case BeaconMessageType.PermissionResponse: {
                ...
                const address: string = await window.electron.beacon.utils.getAddressFromPublicKey(publicKey);
                const appMetadata = await appMetadataManager.getAppMetadata(request.senderId);
                if (!appMetadata) {
                    throw new Error('AppMetadata not found');
                }
