...
import { AppMetadataManager, Logger } from '../../core';
...
export class IncomingRequestInterceptor {
    ...
    private static async getAppMetadata(appMetadataManager: AppMetadataManager, senderId: string): Promise<AppMetadata> {
    ...
    private static async handleV2Message(config: IncomingRequestInterceptorOptionsV2) {
        const { message, connectionInfo, appMetadataManager, interceptorCallback }: IncomingRequestInterceptorOptionsV2 = config;
        ...
        switch (message.type) {
            ...
            case BeaconMessageType.OperationRequest:
                {
                    const appMetadata: AppMetadata = await IncomingRequestInterceptor.getAppMetadata(appMetadataManager, message.senderId);
    ...
    private static async handleV3Message(config: IncomingRequestInterceptorOptionsV3) {
        const { message: msg, connectionInfo, appMetadataManager, interceptorCallback }: IncomingRequestInterceptorOptionsV3 = config;

        const wrappedMessage: BeaconMessageWrapper<PermissionRequestV3<string>> | BeaconMessageWrapper<BlockchainRequestV3<string>> =
            msg as any; /* TODO: Remove any */

        const v3Message: PermissionRequestV3<string> | BlockchainRequestV3<string> = wrappedMessage.message;
