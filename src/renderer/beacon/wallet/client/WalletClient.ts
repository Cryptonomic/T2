...
import { Serializer, Client, LocalStorage, PermissionManager, AppMetadataManager, getSenderId, Logger, NOTIFICATION_ORACLE_URL } from '../../core';
...
export class WalletClient extends Client {
     ...
     */
    private pendingRequests: [BeaconRequestMessage | BeaconMessageWrapper<BeaconBaseMessage>, ConnectionContext][] = [];
    ...
    public async init(): Promise<TransportType> {
        ...
        const p2pTransport: any = new WalletP2PTransport(this.name, keyPair, this.storage, this.matrixNodes, this.iconUrl, this.appUrl);
     ...
     */
    public async connect(newMessageCallback: (message: BeaconRequestOutputMessage, connectionContext: ConnectionContext) => void): Promise<void> {
    ...
    public async getRegisterPushChallenge(backendUrl: string, accountPublicKey: string, oracleUrl: string = NOTIFICATION_ORACLE_URL) {
        // Check if account is already registered
        const challenge: { id: string; timestamp: string } = (await axios.get(`${oracleUrl}/challenge`)).data;

        const constructedString = ['Tezos Signed Message: ', challenge.id, challenge.timestamp, accountPublicKey, backendUrl].join(' ');

        // const bytes = toHex(constructedString);
        const bytes = await window.electron.beacon.utils.toHex(constructedString);
    ...
    public async _connect(): Promise<void> {
        const transport: WalletP2PTransport = (await this.transport) as unknown as WalletP2PTransport;
     ...
     */
    public async addPeer(peer: PeerInfo, sendPairingResponse = true): Promise<void> {
    ...
    public async removeAllPeers(sendDisconnectToPeers = false): Promise<void> {
