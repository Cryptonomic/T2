...
export class AppMetadataManager {
    ...
    public async getAppMetadata(senderId: string): Promise<AppMetadata | undefined> {
        return this.storageManager.getOne((appMetadata: AppMetadata) => appMetadata.senderId === senderId);
    }

    public async addAppMetadata(appMetadata: AppMetadata): Promise<void> {
        return this.storageManager.addOne(appMetadata, (appMetadataElement: AppMetadata) => appMetadataElement.senderId === appMetadata.senderId);
    }

    public async removeAppMetadata(senderId: string): Promise<void> {
        return this.storageManager.remove((appMetadata: AppMetadata) => appMetadata.senderId === senderId);
    }

    public async removeAppMetadatas(senderIds: string[]): Promise<void> {
        return this.storageManager.remove((appMetadata: AppMetadata) => senderIds.includes(appMetadata.senderId));
    }
