...
export class PermissionManager {
    ...
    public async getPermission(accountIdentifier: string): Promise<PermissionInfo | undefined> {
        return this.storageManager.getOne((permission: PermissionInfo) => permission.accountIdentifier === accountIdentifier);
    }

    public async addPermission(permissionInfo: PermissionInfo): Promise<void> {
        return this.storageManager.addOne(permissionInfo, (permission: PermissionInfo) => permission.accountIdentifier === permissionInfo.accountIdentifier);
    }

    public async removePermission(accountIdentifier: string): Promise<void> {
        return this.storageManager.remove((permissionInfo: PermissionInfo) => permissionInfo.accountIdentifier === accountIdentifier);
    }

    public async removePermissions(accountIdentifiers: string[]): Promise<void> {
        return this.storageManager.remove((permission: PermissionInfo) => accountIdentifiers.includes(permission.accountIdentifier));
    }
