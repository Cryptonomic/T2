import { ExposedPromise, getKeypairFromSeed } from '@airgap/beacon-utils';
...
export abstract class BeaconClient {
    ...
    private async initSDK(): Promise<void> {
        ...
        return this.keyPair.then(async (keyPair) => {
            // this._beaconId.resolve(toHex(keyPair.publicKey));
            this._beaconId.resolve(await window.electron.beacon.utils.toHex(keyPair.publicKey));
        });
    }
    ...
    private async loadOrCreateBeaconSecret(): Promise<void> {
        ...
        if (storageValue && typeof storageValue === 'string') {
            ...
            const key = await window.electron.beacon.utils.generateGUID();
