...
const TooltipAdapter = ({ className, offset, ...props }: AdapterProps) => {
    return <RCTooltip {...props} overlayClassName={`${className}__overlay`} prefixCls={`${className}__tooltip`} align={{ offset }} />;
};
...
const StyledTooltip = styled(TooltipAdapter)<
    Pick<TooltipProps, 'arrowPos' | 'className' | 'offset' | 'placement' | 'overlay' | 'boxShadow'> & React.HTMLAttributes<any>
>`
    cursor: pointer;
    ...
    &__tooltip {
        ...
        &-inner {
            ...
            box-shadow: ${({ boxShadow, theme: { colors } }) => (boxShadow ? `0 0 4px 0 ${colors.index0}` : '')};
            -webkit-app-region: no-drag;
        }
...
function Tooltip(props: Props) {
    const { children, className, content, position, arrowPos, offset, boxShadow, ...restOfProps } = props;
    return (
        <StyledTooltip placement={position} className={className} overlay={content} arrowPos={arrowPos} offset={offset} boxShadow={boxShadow} {...restOfProps}>
            {children}
        </StyledTooltip>
    );
}
