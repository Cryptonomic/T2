...
const PageNavButtons = styled.div<{ isactive: boolean }>`
  background-color: ${({ theme: { colors } }) => colors.gray1};
  cursor: ${({ isactive }) => (isactive ? 'pointer' : 'not-allowed')};
  pointer-events: ${({ isactive }) => (isactive ? 'auto' : 'none')};
  color: ${({ isactive, theme: { colors } }) =>
    isactive ? colors.primary : 'rgba(18, 50, 98, 0.2)'};
  height: 100%;
...
const PageNumbers = (props: Props) => {
  ...
  return (
    <Container>
      ...
      <LeftButton isactive={firstNumber !== 0} onClick={() => onClick(currentPage - 1)}>
        <LeftIcon />
      </LeftButton>
      <RightButton isactive={lastNumber < totalNumber} onClick={() => onClick(currentPage + 1)}>
        <RightIcon />
      </RightButton>
    </Container>
  );
